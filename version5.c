...
void dessinerSerpent(int lesX[], int lesY[], char direction);
...
int main()
{
    ...
    while (boucle)
    {
        if (kbhit() == 1)
        {
            ...
            {
                dessinerSerpent(positionX, positionY, direction); // dessiner le nouveau serpent a la position mis a jour
            }
            usleep(vartemporisation);
        }
        else
...
void dessinerSerpent(int lesX[], int lesY[], char direction)
{
    if (direction == DROITE)
    {
        afficher(lesX[0], lesY[0], TETE_DROITE);
    }
    else if (direction == GAUCHE)
    {
        afficher(lesX[0], lesY[0], TETE_GAUCHE);
    }
    else if (direction == HAUT)
    {
        afficher(lesX[0], lesY[0], TETE_HAUT);
    }
    else if (direction == BAS)
    {
        afficher(lesX[0], lesY[0], TETE_BAS);
    }
...
void progresser(int lesX[], int lesY[], char direction, plateau tableau, bool *adr_collision, bool *adr_pomme)
{
    ...
    if ((tableau[lesX[0]][lesY[0]] == MUR) || (tableau[lesX[0]][lesY[0]] == PAVE)) // collision des murs
    {
        *adr_collision = true;
    }
...
void initPlateau(plateau tableau)
{
    ...
    for (int p = 0; p < NUM_PAVES; p++) {
        ...
        do {
            startX = rand() % (TAILLE_MAX_X - LARGEUR_PAVE - 2) + 2;
        ...
        // Place le pavé
        for (int i = 0; i < LARGEUR_PAVE; i++) {
            for (int j = 0; j < TAILLE_PAVE; j++) 
            {
                tableau[startX + i][startY + j] = PAVE;
            }
        }
    }
}
